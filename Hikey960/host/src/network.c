...
void forward_network(network *netp)
{
    ...
    network net = *netp;
    int i;
    int weights_array[100];
    int biaes_array[100];
    printf("[REE] predict start\n");
    double start_time_clock, iElasp;
    start_time_clock = cpuSecond();
    time_t time_s = clock();
    ...
    for(i = 0; i < net.n; ++i){
        net.index = i;
        // printf("[REE] layer: %d, layer type: %d\n", i, net.layers[i].type);
        layer l = net.layers[i];
        l.forward(l, net);
        net.input = l.output;
    }
...
float *network_predict_demo(network *net, float *input)
{
    ...
    double iElasp = cpuSecond() - start_cpu;
    time_t time_e = clock();
    printf("[DEMO]GPU_INDEX = %d\n", net->gpu_index);
    if(net->gpu_index >= 0){
        printf("[DEMO][GPU]ToTal Invoke: (clock, gettimeofday): (%lf,%lf) seconds\n", sec(time_e - time_s), iElasp);
    }
    else
    {
        printf("[DEMO][CPU]ToTal Invoke: (clock, gettimeofday): (%lf,%lf) seconds\n", sec(time_e - time_s), iElasp);
    }
...
void forward_network_demo_gpu_fusion(network *netp)
{
    ...
    // int vgg18_run_fusion[] = {2, 5, 9, 12, 16, 19, 22};
    // int vgg18_run_fusion[] = {0, 3, 16};
    int vgg18_run_fusion[] = {0};
    int resnet50_run_fusion[] = {2, 6, 9, 14, 17, 26, 29, 38, 41, 47};
    int mobilenetv2_run_fusion[] = {-1};
    if (run_demo_idx == 1)
    {
	...
	for(i = 0; i < net.n;){
        ...
        if(tee_idx < num_tee_idxs && i == start_idxs[tee_idx])
        {
            ...
            if (fusion_idx < demo_nums && demo_tee_idxs[fusion_idx] == i){
                run_in_tee = 1;
                // printf("l.type: %d, fusion_idx: %d\n", l.type, demo_tee_idxs[fusion_idx]);
                fusion_idx++;
            }
            l.forward_gpu(l, net);
            net.input = l.output;
