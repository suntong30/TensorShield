...
static TEE_Result test_memory(uint32_t param_types,
                                              TEE_Param params[4])
{
    uint32_t exp_param_types = TEE_PARAM_TYPES( TEE_PARAM_TYPE_MEMREF_INPUT,
                                               TEE_PARAM_TYPE_VALUE_INPUT,
    ...
    float *params0 = params[0].memref.buffer;
    int buffersize = params[0].memref.size / sizeof(float);
    int type = params[1].value.a;
    float *tee_get_buffer = (float *)malloc(sizeof(float) * buffersize);
    for (int _i = 0; _i < buffersize; ++_i)
    {
        tee_get_buffer[_i] = params0[_i];
    }
    if (type == 1)
    {
        read_raw_object_total(tee_get_buffer, buffersize);
        for (int _i = 0; _i < buffersize; ++_i)
        {
            params0[_i] -= tee_get_buffer[_i];
        }
