...
void forward_route_layer_gpu(const route_layer l, network net)
{
    ...
    for(i = 0; i < l.n; ++i){
        ...
        for(j = 0; j < l.batch; ++j){
            copy_offset_gpu(part_input_size, input, j*input_size + part_input_size, 1, l.output_gpu, offset + j*l.outputs, 1);
        }
...
void backward_route_layer_gpu(const route_layer l, network net)
{
    ...
    for(i = 0; i < l.n; ++i){
        ...
        int part_input_size = input_size / l.groups;
        for(j = 0; j < l.batch; ++j){
            axpy_offset_gpu(part_input_size, 1, l.delta_gpu, offset + j*l.outputs, 1, delta, j*input_size + part_input_size, 1);
        }
        offset += part_input_size;
    }
}
