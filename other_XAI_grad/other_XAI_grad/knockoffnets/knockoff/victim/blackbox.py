...
class Blackbox(object):
    ...
    def from_modeldir(cls, model_dir, device=None, output_type='probs'):
        ...
        if "vgg" in model_arch or "alexnet" in model_arch or "resnet50" in model_arch or "mobilenetv2" in model_arch or "resnet" in model_arch:
            model = zoo.get_net(model_arch, modelfamily, pretrained=None, num_classes=num_classes, victim_dir = model_dir , is_victim = True )
        else :
            model = zoo.get_net(model_arch, modelfamily, pretrained=None, num_classes=num_classes, victim_dir = model_dir )
